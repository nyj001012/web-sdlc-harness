...
1. **서브 에이전트 위임 (허브-스포크, Codex)**
   ...
   - 역할을 위임할 때는 그 이름으로 등록된 Codex 커스텀 에이전트(`.codex/agents/<name>.toml`)를 스폰하고, 이번 작업의 범위·대상 파일·완료 조건을 프롬프트에 함께 준다. 등록된 에이전트이므로 정의 파일 내용을 오케스트레이터가 직접 복사해 넣을 필요가 없다 — `developer_instructions`는 Codex가 스폰 시점에 알아서 시스템 프롬프트에 반영한다.
...
7. **쓰기 도구 선택 (Edit vs Write)**
   ...
   - ⛔ **도구 선택은 설정 파일로 강제할 수 없다.** Codex 커스텀 에이전트 TOML에는 Claude의 `tools:`/`allowed-tools:` 같은 도구 화이트리스트 필드가 없다 — 권한은 `sandbox_mode`(`read-only`/`workspace-write`)로만 통제된다. 그래서 부분 수정 원칙은 **자기 규율 + 리뷰어 감사**가 유일한 수단이다. 스폰 프롬프트에서 새 파일을 만들 작업이 아님을 명확히 하고, 리뷰 단계에서 전체 재작성 여부를 확인시킨다.
   - 📌 에이전트의 `sandbox_mode`는 실제 쓰기 여부와 **어긋나지 않게 유지한다.** `code-reviewer`처럼 절대 쓰지 않는 역할만 `read-only`이고 나머지는 `workspace-write`다 — 역할의 쓰기 범위가 바뀌면 이 값도 함께 재검토한다.
...
- ⭐️ **라우트 판별 (주입보다 먼저 수행):** 어떤 선행 명령을 실행할지가 라우트에 따라 갈리므로, 주입·검사에 앞서 라우트를 먼저 확정한다. 아래 페이즈 집합은 **Heavy 기준**이며, Fast 트랙은 다음 항목의 난이도 판별이 다시 줄인다.
  ...
  - **하네스 메타 (Harness-meta):** Phase 2(티켓·브랜치) ➔ Phase 3(오케스트레이터 직접 편집) ➔ Phase 5. Phase 1과 Phase 4는 수행하지 않는다.
    ...
    - **판별 1단계 — 이 저장소가 하네스 자체인가:** 루트 파일이 하네스 자산인지는 저장소의 정체에 달려 있다. 아래 두 신호가 **모두** 참일 때만 하네스 저장소로 본다.
      - `bin/cli.mjs`가 존재한다. 스캐폴더는 `.claude/`·`.codex/`·`.agents/skills/` 하위만 대상 프로젝트로 복사하므로 이 파일은 하네스 저장소에만 있다. 그래서 유효한 식별자다.
...
| 명령 | 용도 |
|---|---|
| `node .codex/tools/inject-scenario.mjs` | `scenario.feature` 전문을 `system-architect`·`issue-pm`·`e2e-tester` 시스템 프롬프트에 주입/갱신 (멱등) |
