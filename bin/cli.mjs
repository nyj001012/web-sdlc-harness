 ...
 *   Codex 호스트는 `.codex/_workspace/`. `--codex` 단독 설치에서는 이제 `.claude/` 디렉터리가
 *   아예 생기지 않는다. 대가는 한 프로젝트 안에서 호스트를 바꿔 가며(예: Claude로 설계하고
 *   Codex로 구현) 같은 design.md를 이어 쓰는 흐름은 지원하지 않는다는 것이다 — 호스트를
 *   바꾸면 그 호스트의 `_workspace/`에 design.md를 다시 확정하거나 수동으로 복사해야 한다.
 *
 * 왜 전역 설치가 아니라 프로젝트별 복사인가:
 *   `inject-design.mjs`는 각 호스트의 에이전트 정의 파일(`.claude/agents/*.md`, `.codex/agents/*.toml`)을 제자리에서 다시 쓴다.
...
/**
 * 호스트(host) 정의. `claude`는 Claude Code, `codex`는 Codex CLI를 가리키며,
 * 값은 프로젝트 루트 기준 디렉터리 이름이다.
 */
const HOST_DIRNAME = { claude: '.claude', codex: '.codex' };
 ...
 * (위 파일 상단 설명 참고 — 워크스페이스는 호스트마다 독립이다).
...
/** 호스트별로 대상 `.gitignore`에 보장해야 하는 런타임 경로. 하네스 저장소의 .gitignore와 같은 근거다. */
const ignoreEntriesFor = (hosts) =>
  hosts.flatMap((host) => [
...
/** 이번 실행에서 다룰 호스트 목록. 옵션이 없으면 둘 다 — "전역 설치"가 기본값으로 두 도구 모두를 지원한다는 요청에 대응한다. */
const HOSTS = WANT_CODEX_ONLY ? ['codex'] : WANT_CLAUDE_ONLY ? ['claude'] : ALL_HOSTS;
...
/**
 * 호스트·코어 디렉터리 이름을 프로젝트 루트 기준 상대 경로로 바꾼다.
 * 대부분은 `<호스트 디렉터리>/<name>`이지만, Codex의 `skills`만 `.agents/skills`로
 * 나간다(파일 상단 "예외" 설명 참고). 이 함수 하나로 그 예외를 흡수해서,
 * 나머지 코드는 호스트 디렉터리를 직접 조립하지 않고 전부 이 함수를 거친다.
 */
function hostPath(host, name) {
  if (host === 'codex' && name === 'skills') return join('.agents', 'skills');
  return join(HOST_DIRNAME[host], name);
}
...
/** 대상에 어떤 호스트든 하네스 코어가 있는지 — update 진입 가능 여부 판단용. */
const isInstalledAnywhere = () =>
  ALL_HOSTS.some((host) => COPY_DIRS.some((d) => existsSync(join(TARGET_ROOT, hostPath(host, d)))));

/** 대상 `<host>/agents/`에 주입 블록이 남아 있는 파일 목록. update 후 재주입 안내에 쓴다. */
...
function mergeGitignore() {
  ...
  const missing = ignoreEntriesFor(HOSTS).filter((e) => !present.has(e));
...
// ─────────────────────────────────────────────────────────────
function init() {
  const byHost = HOSTS.map((host) => ({ host, sources: readSources(host) }));
  ...
  for (const { host, sources } of byHost) {
    for (const { name, files } of sources) {
      for (const f of files) {
        planned += 1;
        if (existsSync(join(TARGET_ROOT, hostPath(host, name), f))) {
          conflicts.push(toPosix(join(hostPath(host, name), f)));
        }
      }
    }
  }
  ...
  if (!DRY_RUN) {
    for (const { host, sources } of byHost) {
      for (const { name, src } of sources) {
        cpSync(src, join(TARGET_ROOT, hostPath(host, name)), { recursive: true, force: true });
      }
    }
    for (const host of HOSTS) {
      for (const parts of ENSURE_DIRS) mkdirSync(join(TARGET_ROOT, HOST_DIRNAME[host], ...parts), { recursive: true });
    }
  }
  for (const { host, sources } of byHost) {
    for (const { name, files } of sources) console.log(`  - ${toPosix(hostPath(host, name))}/ (${files.length}건)`);
  }
  for (const host of HOSTS) {
    for (const parts of ENSURE_DIRS) console.log(`  - ${HOST_DIRNAME[host]}/${parts.join('/')}/ (빈 디렉터리)`);
  }
...
// ─────────────────────────────────────────────────────────────
/**
 * 선택된 호스트들의 코어 트리(agents·skills·tools)를 최신 버전으로 교체한다.
 *
 * 보존 규칙:
...
function update() {
  ...
  const byHost = HOSTS.map((host) => ({ host, sources: readSources(host), injected: injectedAgents(host) }));

  // 패키지에 없는데 대상에는 있는 파일 = 사용자 자산 또는 하네스에서 제거된 고아.
  const orphans = [];
  for (const { host, sources } of byHost) {
    for (const { name, files } of sources) {
      const destDir = join(TARGET_ROOT, hostPath(host, name));
      if (!existsSync(destDir)) continue;
      const known = new Set(files.map(toPosix));
      for (const f of walk(destDir)) {
        if (!known.has(toPosix(f))) orphans.push(toPosix(join(hostPath(host, name), f)));
      }
    }
  }

  const replaced = byHost.reduce((n, s) => n + s.sources.reduce((m, src) => m + src.files.length, 0), 0);
  log(`대상: ${TARGET_ROOT}`);
  log(`호스트: ${HOSTS.join(', ')}`);
  ...
  for (const { host, sources } of byHost) {
    for (const { name, files } of sources) console.log(`  - ${toPosix(hostPath(host, name))}/ (${files.length}건 교체)`);
  }
  for (const host of HOSTS) {
    console.log(`  - ${HOST_DIRNAME[host]}/_workspace/ : 손대지 않았다 (design.md·계약은 사용자 자산)`);
  }
  ...
  console.log('');
  log('\u2713 최신화 완료.');
  const injectedTotal = byHost.reduce((n, s) => n + s.injected.length, 0);
  if (injectedTotal) {
 ...
 * 두 호스트(`.claude/agents`·`.codex/agents`) 모두를 검사한다.
 */
function preflight() {
  const problems = [];

  for (const host of ALL_HOSTS) {
    const agentsDir = join(PKG_ROOT, HOST_DIRNAME[host], 'agents');
  ...
  log('\u2713 preflight 통과 \u2014 두 호스트 모두 주입 블록 0건, 런타임 경로 0건, 의존성 0건, 라이선스 일치, shebang LF');
}
...
옵션:
  --claude         Claude Code 호스트(.claude/)만 설치/최신화한다
  --codex          Codex 호스트(.codex/)만 설치/최신화한다
  (옵션 없음)      기본값. 두 호스트 모두 설치/최신화한다
