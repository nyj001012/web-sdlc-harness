...
```mermaid
%%{init: {'theme': 'neutral', 'flowchart': {'rankSpacing': 70, 'nodeSpacing': 40}, 'config': {'useMaxWidth': true}}}%%
graph TD
...
- **난이도는 라우트와 독립된 축이다** — 라우트가 어느 페이즈를 도는지 정하고, 난이도가 그 페이즈를 얼마나 무겁게 거치는지 정한다. 애매하면 Heavy로 편향한다. 오분류 비용이 비대칭이기 때문이다.
