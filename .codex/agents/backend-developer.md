...
- **수신:** 오케스트레이터의 스폰 프롬프트에 담긴 작업 할당과 (데이터 레인이 있는 라우트에서는) `db-engineer`가 보고한 스키마·마이그레이션 확정 내역, 재반려 시 리뷰어의 피드백
- **발신:** 없음. 코드 작성 완료 사실을 **최종 보고에 담아** 반환하고, 오케스트레이터가 이를 읽어 `code-reviewer`를 스폰한다. 스키마 변경이 필요하면 직접 고치지 않고 그 사실을 최종 보고에 남겨 오케스트레이터가 `db-engineer`를 재스폰하게 한다.
