...
function buildBlock(design) {
  const header = [
    BEGIN,
    `<!-- 자동 생성 영역: \`node ${basename(HOST_DIR)}/tools/inject-design.mjs\`가 관리한다. 직접 편집하지 마라. -->`,
  ];
...
function main() {
  ...
  if (AS_JSON) {
    ...
    if (MODE === 'check' && !summary.ok) {
      console.error(`[inject-design] ✗ 주입 상태가 최신이 아니다. \`node ${basename(HOST_DIR)}/tools/inject-design.mjs\`를 먼저 실행하라.`);
    } else if (MODE === 'check') {
